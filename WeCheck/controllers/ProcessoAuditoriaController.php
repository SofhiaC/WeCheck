...
class ProcessoAuditoriaController {
    ...
    public static function atualizarResultado($idItem, $resultado) {
        ...
        $stmt = $db->prepare($sql);
        $ok = $stmt->execute([$resultado, $idItem]);

        return $ok; // retorna booleano<?php
...
switch ($rota) {
    ...
    public static function salvarNaoConformidade($idItem, $classificacao, $acaoCorretiva, $observacao, $idResponsavel, $dataInicial, $dataConclusao) {
        $db = Database::getConnection();

        $sql = "INSERT INTO tb_nc
